...
def heic_to_png():
    ...
    processed_files = preprocess_heic(input_dir=HEIC_DIR, processed_dir=PNG_DIR)

    for heic_file, input_path_png in processed_files:
        # Load the model
        if MODEL == "detr_object_detection":
            image_processor, model = get_detr_model(model_name=Models.DETR_OBJ.value)
            # # Detect bounding box using DETR
            bboxes = detect_bounding_box_detr(
                image_path=input_path_png, model=model, image_processor=image_processor
            )
            ...
            image_processor, model = get_detr_model(model_name=Models.DETR_SEG.value)
            ...
            if bounding_box:
                ...
                cropped_output_path = os.path.join(
                    OUTPUT_DIR, f"cropped_{file_name}.png"
                )
            ...
            pillow_mask = pipe(
                input_path_png, return_mask=True
            ...
            file_name = heic_file.split(".")[0].split("/")[-1]
            cropped_output_path = os.path.join(OUTPUT_DIR, f"cropped_{file_name}.png")
...
def convert_png_to_jpg(
    input_dir: str = PNG_DIR, output_dir: str = OUTPUT_DIR
) -> List[Tuple[str, str]]:
...
def heic_to_jpeg():
    """Convert HEIC files to JPEG through PNG intermediate format.

    This function performs the following steps:
    1. Convert HEIC to PNG from HEIC input folder to output folder using heic_to_png
    2. Move images from output folder to PNG input folder
    3. Apply convert_png_to_jpg to convert PNG to JPEG
    """
    logger.info("Starting HEIC to JPEG conversion pipeline")

    # Step 1: Convert HEIC to PNG
    logger.info("Step 1: Converting HEIC to PNG")
    heic_to_png()

    # Step 2: Move images from output folder to PNG input folder
    logger.info(f"Step 2: Moving images from {OUTPUT_DIR} to {PNG_DIR}")
    if os.path.exists(OUTPUT_DIR):
        for file in os.listdir(OUTPUT_DIR):
            if file.lower().endswith(".png"):
                src_path = os.path.join(OUTPUT_DIR, file)
                dst_path = os.path.join(PNG_DIR, file)
                shutil.move(src_path, dst_path)
                logger.info(f"Moved {file} to {PNG_DIR}")

    # Step 3: Convert PNG to JPEG
    logger.info("Step 3: Converting PNG to JPEG")
    converted_files = convert_png_to_jpg()

    logger.info(
        f"HEIC to JPEG conversion complete. Processed {len(converted_files)} files"
    )
